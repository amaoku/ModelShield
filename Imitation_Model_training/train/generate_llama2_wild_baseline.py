    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(dev_data), batch_size), total=len(dev_data), unit="batch"):
            ...
            for i in range(len(output_texts)):
                ...
                # res.append({"input":input_text,"predict":predict_text,"target":batch[i]["prediction"],"WMlabel":batch[i]["WMlabel"],"id":batch[i]["id"]})
                res.append({"input":input_text,"predict":predict_text,"baseline_prediction":batch[i]["baseline_prediction"]})
...
def main(args):
    dev_data = load_dev_data(args.dev_file)[504:]#For simplify and save time, we only evaluate ten samples
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dev_file", type=str,default="/data1/pky/BELLE/train/baseline_wild_train_data.json")
    parser.add_argument("--model_name_or_path", type=str, default='/data1/pky/output/baseline/trained_model_llama2-wild-baseline', help="pretrained language model")
    ...
    parser.add_argument("--output_file", type=str, default="/data1/pky/output/baseline/1118-wild-llama2-WM-baseline_all_1202_start_504.json")
